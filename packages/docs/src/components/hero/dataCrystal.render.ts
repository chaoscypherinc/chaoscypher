/**
 * Canvas rendering utilities for the hero data-crystal background.
 *
 * Ported from packages/interface/src/components/graphConstellation/canvasRendering.ts
 * with: hexToRgba inlined (no theme dependency), FOV=1200 (flatter perspective),
 * PSCALE_CAP=1.9 (hard cap for ultra-wide viewports).
 *
 * Pure generator/utility functions that don't depend on React state.
 */

/** Hex (#rrggbb) + alpha -> rgba() string. Inlined so docs has no theme dependency. */
...
/** Perspective field-of-view — flatter than the dashboard's 600 for a calmer hero. */
const FOV = 1200;                 // hero: flatter than the dashboard's 600
const NEAR_CLIP_DENOM = FOV * 0.1;
const PSCALE_CAP = 1.9;           // hard cap so nodes never fly off ultra-wide viewports
...
export function project3D(
  ...
  let pScale = FOV / Math.max(NEAR_CLIP_DENOM, FOV + tz * layoutScale);
  if (pScale > PSCALE_CAP) pScale = PSCALE_CAP;
