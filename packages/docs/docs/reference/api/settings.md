...
| `instance_id` | string | No | Target Ollama instance; pulls to **all** instances when omitted |
...
```
data: {"status": "downloading", "completed": 1048576, "total": 4800000000, "instance_id": "default"}
data: {"status": "downloading", "completed": 2097152, "total": 4800000000, "instance_id": "default"}
data: {"status": "success", "instance_id": "default"}
```

Error events carry `{"status": "error", "error": "...", "instance_id": "..."}`.
...
| `instance_id` | string | No | Target Ollama instance; removes from **all** instances when omitted |
...
The `instance_id` query parameter selects which Ollama instance to query
(default `"default"`).
...
Returned by the synchronous reset and seed endpoints. The queued endpoints
(`cleanup/orphans`, `reset/knowledge`, `reset/all`) instead return
`QueuedResetResponse` (`{task_id, operation_type}`) at `202 Accepted`.
