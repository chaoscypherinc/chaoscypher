...
For the public-repo launch the 50-file chain (`0001`–`0050`) was squashed into a single consolidated `0001` baseline, regenerated from `SQLModel.metadata` via `alembic revision --autogenerate` against an empty database. The revision id stays `0001` and `down_revision` stays `None`, so the self-healing startup runner's `_BASELINE_REVISION` is unchanged.

Databases created before the squash — recorded at a now-deleted revision such as `0050_…` — are **unsupported**. The squash predates v0.1.0, so no released build ever wrote one. Since 2026-08-14 `ensure_stamped` raises `UnsupportedDatabaseLineageError` for any revision absent from the script directory, and Cortex exits with that message instead of starting. The message names the revision and covers both causes an unresolvable id can have — the retired lineage, or a database stamped *ahead* of the build after a downgrade — leading with the non-destructive fix ("re-install the newer version") before the back-up-then-re-create/export path, because the check cannot tell the two apart from an id alone.

Re-stamping such a database at the baseline — the behaviour up to 2026-08-14 — was not a recovery. It only relabelled `alembic_version`; the shipped `0002`+ migrations then replayed against a schema they were never written for, and the boot died in the post-migration drift gate with an opaque `SchemaIntegrityError`, restarting on every supervisor retry. Failing once, loudly, is the supported behaviour.
