...
- **Container blast radius.** Cortex and Neuron run as `appuser` with `no-new-privileges`. The supervisor password file is root-only at `/data/secrets/supervisor_password` (`root:root 0600`), so an `appuser` process cannot read or rewrite the file; the value itself is exported into the environment of supervisord and its `cortex` and `neuron` children, which need it to reach supervisord's RPC endpoint, so any `appuser` process can read it from `/proc/<pid>/environ`. It is a local service credential, not a cross-tenant boundary — the supervisord socket is already `appuser`-reachable by design. Other deployment secrets under `/data/secrets/` are owned `root:appuser 0640` — readable by services that need them, but a compromised appuser process cannot tamper with them. Static nginx config lives in root-owned `/etc/nginx/conf.d`; only ephemeral runtime state lives in the appuser-writable `/run/chaoscypher/` dir.
- **Common archive attacks.** Archive extraction — the source-loader path for user uploads and the `.ccx` package import path — enforces path containment (`is_relative_to`), a total decompressed size cap, a member-count cap, and rejects symlinks, devices, and FIFOs. The two are separate implementations enforcing the same limits rather than one shared helper. Both caps are enforced **per extraction call**, not cumulatively: a nested archive gets a fresh budget, and nesting is bounded by `max_nesting_depth` (default 3) rather than by a cumulative byte ceiling. Database backups are plain SQLite file copies, not archives, so no extraction happens on restore.
...
- **CPU exhaustion through the API-key auth path.** nginx runs `/auth/verify` as an `auth_request` subrequest on every `/api/` request, so the `Authorization: Bearer` arm is reachable by any unauthenticated LAN host at the full permitted request rate. It does not bcrypt-scan stored hashes: each key is indexed by a keyed HMAC selector, so an unrecognised key is rejected after one hash computation and no bcrypt work. Keys minted before selectors existed are the one exception — until such a key has been presented once and migrated, it stays in a small scanned set, so revoke old keys you no longer use rather than leaving them to be scanned. On top of that, 5 failed bearer verifies from one client address within 60 seconds block that client's bearer attempts for 60 seconds. The nginx rate limit deliberately stays off this location — it fires on every legitimate `/api/` call too — so the cap lives in Cortex where it can be scoped to bearer *failures*: session-cookie verification and valid keys are never throttled. The throttle is per Cortex process and keys on the client address that the matching nginx config forwards; run Cortex behind an older edge config and it degrades to a single shared bucket (see [Upgrading](../getting-started/upgrading.md)) — bearer auth pauses for everyone for 60 seconds instead of for the offending address, which is a nuisance, not a bypass. The same degraded mode applies behind any *additional* proxy — a Cloudflare Tunnel, Caddy, or Tailscale Serve in front of nginx — because nginx then sees that proxy as `$remote_addr` and forwards the same address for every caller, so all callers again share one bucket; teaching nginx the true client address via `real_ip_header`/`set_real_ip_from` is future work.
...
| **Online password brute-force beyond bcrypt + nginx rate-limit.** | Account lockout would lock the owner out. Self-hosted single-user attacker doesn't get many guesses through `5r/m` + bcrypt. This applies to the **password login** route (`/api/v1/auth/login`) only. The API-key bearer path is a different shape — a `cc_live_` key is 192 bits of random, not a guessable secret — and is covered by the selector lookup and the per-client bearer throttle described above, not by this `5r/m` limit. | Set a 16+ char password; put the box behind a VPN or Tailscale; don't expose `0.0.0.0` without TLS. |
